...
def search_results(request):
    ...
    return {
        'page_title': u'Поиск : Результаты : %s' % (settings.SITE_TITLE,),
...
def category(request, title=None):
    ...
    return {
        'page_title': u'Категория : %s : %s' % (category, settings.SITE_TITLE),
        'mode': 'list',
        'this': category,
        'items': items,
...
def collection(request, id=None):
    ...
    return {
        'page_title': u'Коллекция : %s' % (settings.SITE_TITLE, ),
        'mode': 'list',
        'this': collection,
        'items': items,
        'url': '/collection/%s/' % (id,),
...
def item(request, id=None):
    ...
    return {
        'page_title': u'Товар : %s : %s' % (item, settings.SITE_TITLE),
        'mode': 'item',
        'this': item.category,
...
def show_cart(request):
    return {
        'page_title': u'Корзина : %s' % (settings.SITE_TITLE, ),
        'mode': 'cart',
...
def show_order(request):
    ...
    if request.method == 'POST' and form.is_valid():
        try:
            ...
            error_desc = e
    return {
        'page_title': u'Заказ : %s' % (settings.SITE_TITLE, ),
        'form': form,
        ...
        if conn:
            ...
            if auth:
                for recipient in getattr(settings, 'JABBER_RECIPIENTS', None):
                    ...
                    time.sleep(1)
    return {
        'page_title': u'Заказ принят : %s' % (settings.SITE_TITLE, ),
        }
